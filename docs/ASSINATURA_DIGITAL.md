...
assinou, quando, com um código e um hash de integridade) e o PDF em
`uploads/certificates` é **regerado** com o bloco de assinatura **centralizado**
no corpo do certificado (onde antes ficavam as linhas). Não usa certificado
criptográfico (.pfx/ICP‑Brasil).
...
- **`storage/certificate-file-storage.service.ts`**: métodos para ler e
  sobrescrever o PDF existente na pasta.
- **`signature/certificate-signature.service.ts`**: orquestra a assinatura em
  lote (só organizador), re‑renderiza o PDF com a assinatura e faz a verificação
  pública.
...
- Depois de assinar, o mesmo `arquivo_pdf` (mesma URL) é sobrescrito com o PDF
  já assinado — o download que o front usa não muda.
